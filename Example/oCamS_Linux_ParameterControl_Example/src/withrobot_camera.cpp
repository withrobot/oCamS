...
bool Camera::get_capability()
{
    ...
    DBG_PRINTF("V4L2_CAP_VIDEO_OUTPUT : %s", (v4l2_s.capability.capabilities & V4L2_CAP_VIDEO_OUTPUT) ? "True" : "False");
...
bool Camera::set_outbuffer()
{
    /* request buffers */
    memset(&v4l2_s.requestbuffers, 0, sizeof(v4l2_s.requestbuffers));
    v4l2_s.requestbuffers.count = WITHROBOT_CAMERA_REQUEST_BUFFER_COUNT;
    v4l2_s.requestbuffers.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
    v4l2_s.requestbuffers.memory = V4L2_MEMORY_MMAP;
    if (xioctl(VIDIOC_REQBUFS, &v4l2_s.requestbuffers) == -1) {
        DBG_PERROR("request buffer");
        return false;
    ...
    for (unsigned int i = 0; i < buffer_count; i++) {
        ...
        v4l2_s.buffer.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
        v4l2_s.buffer.memory = V4L2_MEMORY_MMAP;
        v4l2_s.buffer.index = i;
...
bool Camera::start_out()
{
    ...
    v4l2_s.buf_type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
    if (xioctl(VIDIOC_STREAMON, &v4l2_s.buf_type) == -1) {
        DBG_PERROR("VIDIOC_OUTPUTON");
        return false;
    }
...
int Camera::set_frame(unsigned char* in_buffer, const unsigned int size, unsigned int timeout_sec)
{
#if 0
    fd_set fds;
    FD_ZERO(&fds);
...
#endif
    return write_buffer(in_buffer, size);
}
...
int Camera::write_buffer(unsigned char* buffer, const unsigned int size)
{
    int retval = -1;
    ...
    if (v4l2_s.buffer.bytesused == size || v4l2_s.format.fmt.pix.pixelformat == V4L2_PIX_FMT_MJPEG) {
        retval = v4l2_s.buffer.bytesused;
        memcpy(buffers[v4l2_s.buffer.index].buffer, buffer, v4l2_s.buffer.bytesused);
    }
    else {
        DBG_PRINTF("Warning :: Different buffer size. v4l2: %d, User: %d", v4l2_s.buffer.bytesused, size);
    }
